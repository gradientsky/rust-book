...
- Covers: Iterator trait (`next`, `Item`), three iteration modes (iter/iter_mut/into_iter), for-loop desugaring, lazy evaluation, adaptors (map, filter, filter_map, enumerate, zip, take, skip, flatten, flat_map, chain, **`windows`** (sliding windows), inspect), consumers (collect, fold, sum, product, count, any, all, find, position, min, max, min_by_key, max_by_key), **`total_cmp` for float sorting** (PartialOrd vs Ord, NaN safety, `sort_by`/`max_by` patterns), collecting into Vec/String/HashMap/HashSet/Result, ranges as iterators, std::iter constructors (repeat_n, once, from_fn, successors), implementing Iterator for custom types (Countdown), IntoIterator for custom collections (Playlist), `is_sorted_by_key` (stable 1.82)
...
- All 30 code examples verified to compile and produce documented output (Rust 1.93.1, edition 2024)
...
- **Replaced unstable `array_windows::<N>()` with stable `windows()`**: `array_windows` is still nightly-only as of Rust 1.93.1 (tracking issue #75027); replaced "Array Windows" B-head with "Sliding Windows" using stable `windows()` method; same teaching examples (day-over-day changes, moving averages) now use indexing instead of array destructuring; 2 examples verified zero-warning Rust 1.93.1
- Deliberately omitted: `Iterator::array_chunks` (nightly-only), `Iterator::map_windows` (nightly-only), `slice::array_windows` (nightly-only, tracking issue #75027), `gen` blocks (nightly-only), `IntoIterator for Box<[T]>` 2024 behavior change (too niche), `try_fold`/`ControlFlow` (advanced), `Extend` trait (Part 5), `ExactSizeIterator`/`DoubleEndedIterator` (advanced traits better for Part 7), `iter::chain` free function (1.91, not needed when method form exists)
- **Review items:**
...
- All 12 compilable code examples verified zero-warning (Rust 1.93.1, edition 2024); 2 does_not_compile examples verified (E0373 borrow in thread, E0277 Rc not Send); 5 `rust,ignore` examples (spawn signature, async/tokio)
...
- **6.1**: ~~`std::io::pipe()` (1.87) — cross-platform anonymous pipes in std~~ DONE
- **4.3**: ~~`slice::array_windows::<N>()` — still nightly-only as of 1.93.1; replaced with stable `windows()`~~ DONE
