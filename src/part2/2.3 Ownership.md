...
types that get duplicated automatically, so they never move. You will see the
`String` vs `&str` distinction explored more deeply in the next chapter on
...
need both `s1` and `s2` to be valid, you can clone the value — you will see how
in the Clone section later in this chapter.
...
- **No data races.** Ownership ensures that a value has exactly one owner at
  a time. Combined with the borrowing rules you will learn next, this guarantee
  extends to concurrent programs — the same system that prevents use-after-free
  also prevents data races.
