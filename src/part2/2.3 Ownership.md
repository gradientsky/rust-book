...
### Try It: Move Semantics

These short exercises practice moving ownership through function calls, return
values, and closures, each solvable in a single `main`.

#### Exercise 2.3-A1: The Label Builder

Write a function `build_label(prefix: String, body: String) -> String` that
takes ownership of both strings, joins them as `"prefix: body"`, and returns the
result. In `main`, create a `prefix` and a `body`, pass both into `build_label`,
and print the returned label along with its length.

Because the function takes both strings _by value_, ownership moves into it — the
original `prefix` and `body` bindings are invalid afterward, and the only way to
get data back is through the return value.
...
```rust
fn build_label(prefix: String, body: String) -> String {
...
fn main() {
    let prefix = String::from("status");
    let body = String::from("online");

    let label = build_label(prefix, body);
    // prefix and body are no longer valid — both moved into build_label
    println!("{label}");
    println!("len: {}", label.len());
}
```
...
The `prefix` parameter is rebound to a mutable `label` inside the function (no
clone — the heap buffer simply changes owner), `body` is appended, and the
finished `String` moves back out through the return value to become the new
`label` in `main`.

</details>

#### Exercise 2.3-A2: The Stamp Factory

Write a function `make_stamp(tag: String) -> impl Fn()` that returns a `move`
closure. The closure must own `tag` so it can outlive the factory call, printing
`[tag] event recorded` each time it runs. In `main`, build one stamp from
`"AUDIT"` and call it three times.

Without `move`, the closure would try to borrow `tag` from a scope that
disappears the moment `make_stamp` returns. With `move`, ownership of the
`String` travels into the closure, so it stays valid for every call.
...
```rust
fn make_stamp(tag: String) -> impl Fn() {
    move || println!("[{tag}] event recorded")
}

fn main() {
    let stamp = make_stamp(String::from("AUDIT"));
    // tag moved into the closure — the closure now owns it
...
The `move` keyword transfers `tag` into the closure exactly the way passing it
to a function would. The closure now owns the `String` and can read it as many
times as it likes — here, on all three calls.
...
heap allocation with its own copy of the text — `.clone()` allocates a new
buffer on the heap and copies the bytes into it. Modifying one would not
affect the other.
...
### Try It: Copy, Clone, and Drop

These short exercises practice the three ownership behaviors — automatic copies,
explicit clones, and deterministic drops — each solvable in a single `main`.

#### Exercise 2.3-B1: Copy Versus Clone

In one `main`, show the difference between a `Copy` type and a clone. First make
a tuple `point = (3, 4)` and assign it to `copy` — because both elements are
`Copy`, the tuple is `Copy`, so `point` stays valid. Then make a
`String` and produce an independent owner with `.clone()`. Print both originals
alongside their duplicates.

The tuple is duplicated automatically on assignment, so no `.clone()` is needed.
The `String` is _not_ `Copy`, so a plain assignment would move it — `.clone()`
is what gives you two independent owners.

Expected output:

```
original: (3, 4), copy: (3, 4)
original: data, duplicate: data
```
...
```rust
fn main() {
    // point is a tuple of two Copy types, so the tuple is Copy
    let point = (3, 4);
    let copy = point;
    println!("original: {point:?}, copy: {copy:?}");

    // String is not Copy — clone makes an independent owner
...
A tuple of `Copy` values is itself `Copy`, so `let copy = point` duplicates it
bit-for-bit and leaves `point` usable. `String` owns a heap buffer, so it can
never be `Copy` — duplicating it requires the explicit `.clone()`, which
allocates a fresh buffer and copies the bytes. (The `{point:?}` form is debug
formatting, the same `{:?}` you use to print a `Vec`.)

</details>

#### Exercise 2.3-B2: Drop on Reassignment

Use a tiny instrumented type to see _when_ a value is dropped. Define a `Guard`
struct with a `name: &'static str` field and a `Drop` impl that prints
`releasing {name}`. In `main`, bind a mutable `slot` to a `"first"` guard, print
`holding first`, then reassign `slot` to a `"second"` guard and print
`holding second`. Finish with `end of main`.

Before running it, predict where `releasing first` appears. Reassigning a
mutable variable drops the _previous_ value immediately — not at the end of the
scope — because ownership rule two allows only one owner at a time.
...
fn main() {
    let mut slot = Guard { name: "first" };
    println!("holding {}", slot.name);

    slot = Guard { name: "second" };
    println!("holding {}", slot.name);
...
The instant `slot` receives the `"second"` guard, nothing owns `"first"` anymore,
so Rust drops it right there — `releasing first` prints _before_ `holding
second`. The `"second"` guard then lives until the closing brace of `main`, where
it is dropped normally.
...
**What ownership does _not_ do.** Ownership prevents double frees and
use-after-free, but it does not compact the heap. Unlike garbage-collected
languages (Java, Go, C#), Rust cannot move heap objects behind your back —
every reference is a real memory address, and there is no runtime to update
pointers. Heap fragmentation is handled by the system allocator (e.g.,
jemalloc, mimalloc), not by the language. You also do not need reference
counting by default — single ownership and deterministic drops handle most
cases. Reference counting (`Rc<T>`) exists for the rare situations where
multiple owners are genuinely needed, and is covered in a later chapter.
...
### Exercise 2.3-1: The Ownership Relay
...
5. After calling `add_greeting(name)`, try adding `println!("{name}")` and
   read the compiler error. Remove the line once you understand why it fails.
...
**Stretch goal.** Put the three functions in an array and use `.iter().fold()`
to chain them in a single expression. The moves still happen — `fold` moves
the accumulator into each closure call and the return value becomes the next
accumulator.

</details>

### Exercise 2.3-2: The Playlist Manager
...
> **Hints.** Use `.iter().enumerate()` to get both the index and the song in
> `show_and_return`. To debug-print a `Vec`, use `{:?}` in the format string —
> `Vec` implements `Debug` but not `Display`.
>
...
**Common pitfall.** You might be tempted to use `.fold()` in `show_and_return`
with a counter as the accumulator and `println!` as the body. This works, but
`fold` is meant for producing a value — use `enumerate` with `for` or
`for_each` when the goal is a side effect like printing.

</details>

### Exercise 2.3-3: The Cleanup Log
...
2. Write a function `Connection::new(label)` that prints `[open] {label}` and
   returns a `Connection`. (This is a function defined inside `impl Connection`
   that does not take `self` — you call it as `Connection::new("database")`
   rather than on an existing value.)
...
4. In `main`, create a `"database"` connection in the outer scope. Then open
   an inner block `{ }`, print `"-- inner scope work --"`, and create
   `"cache"` and `"logger"` connections inside it. After the inner block
   closes, print `"-- outer scope work --"`.
...
### Exercise 2.3-4: The Lease Office

**Goal.** Model a property office where renewal reminders are built as `move`
closures that own their data, and signed leases are released deterministically
when ownership flows into a function that consumes them. This ties together
move-into-closure, function factories, and custom `Drop`.

**Concepts practiced:** `move` closures capturing an owned `String`, returning
a closure with `impl Fn`, function factories, `struct` with `impl Drop`, an
associated `new` function returning `Self`, moving a `Vec` into a function that
consumes (and thus drops) it, `String::from`, `Vec`/`vec!`, `.iter()`,
`saturating_add`, and early `drop()`.

**Your task:**

1. Create a `Lease` struct with a `name: String` field and a `days: u32` field.
2. Give it an associated function `Lease::new(name: &str, days: u32) -> Self`
   that prints `[sign]   {name} for {days} days` and returns the `Lease`.
3. Implement the `Drop` trait so cleanup prints `[release] {name} ({days} days)`.
4. Write `make_renewal_reminder(tenant: String, days: u32) -> impl Fn()` — a
   factory that returns a `move` closure. The closure must own `tenant` so it
   can outlive the factory call, printing
   `Reminder: {tenant}'s lease renews in {days} days` each time it runs.
5. Write `total_days(leases: Vec<Lease>) -> u32` that takes ownership of the
   vector, sums every lease's `days` with `saturating_add`, and returns the
   total. Because the function owns the vector, every `Lease` is released when
   the function ends.
6. In `main`, build a reminder for `"Ada"` (30 days), then a `vec!` of three
   leases: `"Ada"` (30), `"Grace"` (45), `"Linus"` (15). Call the reminder
   twice, then `drop()` it explicitly. Print `-- midyear audit --`, move the
   vector into `total_days`, print the total, and finish with
   `-- offices closed --`.

> **Key insight.** The reminder closure owns its `String`, so dropping the
> closure frees that text — that is why `drop(remind)` produces no `[release]`
> line of its own. The three `[release]` lines all appear _before_ `total_days`
> returns, because moving the vector into the function makes the function the
> owner, and a `Vec` drops its elements in order as the function scope ends.

**Expected output:**

```
[sign]   Ada for 30 days
[sign]   Grace for 45 days
[sign]   Linus for 15 days
Reminder: Ada's lease renews in 30 days
Reminder: Ada's lease renews in 30 days
-- midyear audit --
[release] Ada (30 days)
[release] Grace (45 days)
[release] Linus (15 days)
Total booked: 90 days
-- offices closed --
```
...
impl Lease {
    fn new(name: &str, days: u32) -> Self {
        println!("[sign]   {name} for {days} days");
        Lease {
...
impl Drop for Lease {
    fn drop(&mut self) {
        println!("[release] {} ({} days)", self.name, self.days);
    }
}

fn make_renewal_reminder(tenant: String, days: u32) -> impl Fn() {
    move || println!("Reminder: {tenant}'s lease renews in {days} days")
}

fn total_days(leases: Vec<Lease>) -> u32 {
    let mut sum: u32 = 0;
    for lease in leases.iter() {
        sum = sum.saturating_add(lease.days);
    }
    sum
} // leases is dropped here — every Lease is released

fn main() {
    let remind = make_renewal_reminder(String::from("Ada"), 30);
    ...
    remind();
    remind();
    drop(remind); // the closure owns "Ada" — free it now

    println!("-- midyear audit --");
    let booked = total_days(leases);
    // leases is no longer valid — it moved into total_days
    println!("Total booked: {booked} days");

    println!("-- offices closed --");
}
```
...
Three ownership ideas drive this program. First, `make_renewal_reminder` uses
`move` so the returned closure _owns_ `tenant`; without `move`, the closure
would try to borrow a `String` from a scope that no longer exists once the
factory returns. Second, `drop(remind)` frees the closure — and with it the
captured `"Ada"` text — without printing anything, because a plain `String`
has no custom `Drop`. Third, passing `leases` to `total_days` _moves_ the whole
vector; the function becomes the sole owner, so all three `[release]` lines fire
as the function scope ends, before `booked` ever comes back to `main`. After
that move, `leases` is invalid in `main` — the compiler would reject any attempt
to use it again.

**Stretch goal.** Add a second reminder for `"Grace"` and store both closures in
a `Vec`. You will find you cannot push two different `impl Fn` closures into the
same `Vec<_>` without a tool you have not learned yet — each closure has its own
unique type. For now, keep them in separate variables and call each directly.
...
constantly — exactly the friction you felt in Exercise 2.3-2. References solve
this elegantly.
