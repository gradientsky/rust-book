...
Figure 2-4 shows the owner, the borrowed reference, and the heap data it still
owns.
...
- `longest_title` takes a `&[String]` and returns a `&str` — the longest song
  title in the playlist. This practices returning a borrowed slice from borrowed
  input.
- `total_characters` takes a `&[String]` and returns the sum of all title
  lengths as a `usize`.
- `display_playlist` takes a `&[String]` and prints a numbered list.

In `main`, create a `Vec<String>` of four songs. Call all three functions, then
print the playlist's length to confirm it is still owned by `main`.
...
> no need to return the `Vec` to keep using it. Borrowing eliminates the
> take-and-return choreography that ownership alone requires.

<details>
<summary>Solution</summary>

```rust
fn longest_title(playlist: &[String]) -> &str {
    let mut longest = &playlist[0];
    for title in &playlist[1..] {
        if title.len() > longest.len() {
            longest = title;
        }
    }
    longest
...
fn total_characters(playlist: &[String]) -> usize {
    let mut total = 0;
...
fn display_playlist(playlist: &[String]) {
    println!("Playlist ({} songs):", playlist.len());
    for (i, title) in playlist.iter().enumerate() {
        println!("  {}. {title}", i + 1);
    }
}
...
fn main() {
    ...
    let longest = longest_title(&playlist);
    println!("Longest title: {longest}");

    let chars = total_characters(&playlist);
...
`longest_title` returns `&str` — a slice borrowed from the data inside the
`Vec`. The compiler infers that the returned reference lives as long as the input
slice, so no explicit lifetime annotation is needed.
