...
### Try It: Shared and Mutable References

These short exercises practice the difference between `&` and `&mut`, each solvable in a single `main`.

#### Exercise 2.4-A1: Read Then Append

Start with `let mut word = String::from("rust")`. Take two shared references and print the value and its length through them. Then, after the shared borrows are last used, append an exclamation mark through a `&mut String` parameter. The function receives `s: &mut String`; use `s.push('!')` to add the character in place.
...
fn main() {
    ...
    // shared borrows: read the value without changing it
    let r1 = &word;
    let r2 = &word;
    println!("before: {r1} (len {})", r2.len());

    // the shared borrows have ended; a mutable borrow is now allowed
    add_exclamation(&mut word);
...
The two shared references `r1` and `r2` coexist freely — many readers are allowed. Their borrows end at the `println!`, so the `&mut word` passed to `add_exclamation` does not conflict with them. A shared and a mutable borrow of `word` never overlap.

</details>

#### Exercise 2.4-A2: Adjust a Balance Through a Mutable Reference

Start with `let mut balance = 100`. Create a mutable reference `account` and use the dereference operator `*` to subtract 30 and to read the result. Then call a function `add(target: &mut i32, amount: i32)` that does `*target += amount` to add 50. Finally, print the balance directly.
...
```rust
fn add(target: &mut i32, amount: i32) {
    *target += amount;
}
...
fn main() {
    ...
    // mutable reference: follow it with `*` to read and write the value
    let account = &mut balance;
    *account -= 30;
    println!("after withdrawal: {}", *account);

    // call a function that takes a &mut i32
    add(&mut balance, 50);

    // the &mut borrow has ended, so a shared read is fine again
...
With a primitive like `i32`, you reach the value behind a mutable reference with `*` — `*account -= 30` both reads and writes. The borrow held by `account` ends after its last use in the first `println!`, which is why `&mut balance` can be created again for the `add` call, and why the final `balance` read is allowed afterward.
...
> **Tip**
>
> A common pattern when slicing at a delimiter: use
> `text.find(' ').unwrap_or(text.len())` to get the position of the first
> space, or the end of the string if there is none. Then `&text[..pos]` gives
> you the first word in both cases — no `if let` needed. Be careful with
> `unwrap_or(0)` — that produces an empty slice, not the whole string.
...
> **Tip**
>
> `split_at(n)` splits a slice or string into two parts at byte index `n`,
> returning both halves. It does _not_ remove anything — if you split at a
> space, the second half starts with that space. To skip the delimiter, advance
> by one byte: `&remainder[1..]`.

### Try It: Slices

These short exercises practice borrowing sequences as slices, each solvable in a single `main`.

#### Exercise 2.4-B1: Largest in a Slice

Write `largest(numbers: &[i32]) -> i32` that returns the biggest value, walking the slice with `for &n in &numbers[1..]` and tracking the maximum. In `main`, build an array `[42, 17, 99, 73, 8]`. Call `largest` once on the whole array (`&scores`) and once on a sub-range (`&scores[1..4]`), then confirm the array is still usable by printing its length.

Expected output:

```
largest of all: 99
largest of middle: 99
still have 5 scores
```
...
fn main() {
    let scores = [42, 17, 99, 73, 8];

    // &scores borrows the whole array as a &[i32]
    println!("largest of all: {}", largest(&scores));

    // &scores[1..4] borrows only a sub-range
    println!("largest of middle: {}", largest(&scores[1..4]));

    // the array was only borrowed, so it is still usable
...
One function works on both the whole array and a sub-range because `&[i32]` does not care how the data was allocated — `&scores` and `&scores[1..4]` both produce a slice. The sub-range `[1..4]` covers indices 1, 2, and 3 (`17, 99, 73`), so its largest is still `99`. Each call only borrows, so `scores` remains available afterward.

</details>

#### Exercise 2.4-B2: First Word from Any Text

Write `first_word(text: &str) -> &str` that returns the text up to the first space, or the whole string if there is none. Use `text.find(' ')` with `if let`, and slice with `&text[..space]`. In `main`, call it on a string literal and on a borrowed `String`, then print the `String` to prove it was only borrowed.

Expected output:

```
first word of literal: Borrowing
first word of String: slices
original String: slices avoid copying
```
...
fn main() {
    // first_word accepts &str, so it works on a literal directly
    let headline = "Borrowing keeps the owner intact";
    println!("first word of literal: {}", first_word(headline));

    // ...and on a borrowed String, which converts to &str automatically
    let owned = String::from("slices avoid copying");
    let word = first_word(&owned);
    println!("first word of String: {word}");

    // the String was only borrowed — still available
    println!("original String: {owned}");
}
```
...
Taking `&str` instead of `&String` lets `first_word` accept both a string literal and a `&String` — Rust converts the `&String` to `&str` for free. The returned `&str` is a window into the caller's data, with no allocation, and because the input was only borrowed, `owned` is still usable on the last line.
...
### `iter()` vs `iter_mut()` in Borrowing Terms

The shared-or-mutable distinction extends to iterators. `iter()` yields `&T`
(shared references) — you can read but not modify. `iter_mut()` yields `&mut T`
(mutable references) — you can modify in place:

```rust
fn main() {
    let mut prices = vec![10.0, 20.0, 30.0];

    // iter(): shared borrows — read only
    let total: f64 = prices.iter().map(|p| p * 1.1).sum();
    println!("projected total: {total}");

    // iter_mut(): mutable borrows — modify in place
    prices.iter_mut().for_each(|p| *p *= 1.1);
    println!("updated: {prices:.1?}");
}
```
...
```
projected total: 66
updated: [11.0, 22.0, 33.0]
```

A common mistake is using `iter().map(...).collect()` when you want in-place
modification. That creates a _new_ collection — it never writes back to the
original. For in-place changes, use `iter_mut()` with `for_each` or a `for`
loop.

### Try It: Working with the Borrow Checker

These short exercises practice patterns that keep conflicting borrows from overlapping, each solvable in a single `main`.

#### Exercise 2.4-C1: Read Phase Then Write Phase

Start with `let mut temps = vec![68, 71, 70, 69, 72]`. First, in a read phase, find the maximum by copying values out with indexing (`temps[i]`) into a `mut max` — `i32` is `Copy`, so no borrow lingers. Print the max. Then, in a write phase, use `iter_mut()` to add one degree to every reading in place, and print the result.

Expected output:

```
max before: 72
after warming: [69, 72, 71, 70, 73]
```
...
```rust
fn main() {
    let mut temps = vec![68, 71, 70, 69, 72];

    // read phase: find the max by copying values out (i32 is Copy)
    let mut max = temps[0];
    ...
    // the read borrows have ended; now we may borrow mutably
    println!("max before: {max}");

    // write phase: bump every reading by one degree, in place
    for t in temps.iter_mut() {
        *t += 1;
    }

    println!("after warming: {temps:?}");
}
```
...
Indexing with `temps[i]` borrows only for the instant of each access, and copying the `i32` into `max` leaves no live borrow once the read phase ends. That is what lets the write phase take `temps.iter_mut()` — a `&mut i32` per element — without colliding with the earlier reads. Finish reading before you start writing.

</details>

#### Exercise 2.4-C2: Swap Neighbors with Indices

Do one pass of bubble sort over `let mut data = vec![5, 3, 8, 1, 9, 2]`. Loop `for i in 0..data.len() - 1`; when `data[i] > data[i + 1]`, call `data.swap(i, i + 1)`. Because indexing borrows only for each access, reading the two neighbors and calling the mutating `swap` never conflict. Print the result.

Expected output:

```
after one pass: [3, 5, 1, 8, 2, 9]
```
...
fn main() {
    ...
    // One pass of bubble sort: read two neighbors and write through swap.
    // Indexing borrows only for the instant of each access, so reading
    // data[i] and data[i + 1] never conflicts with data.swap().
    for i in 0..data.len() - 1 {
        if data[i] > data[i + 1] {
            data.swap(i, i + 1);
        }
...
A single `&mut` reference to one element could not also let you read its neighbor — that would be two live borrows of `data` at once. Indexing sidesteps the conflict: `data[i]` and `data[i + 1]` each borrow only for the instant of the comparison, and `data.swap(i, i + 1)` borrows mutably only for the swap. One pass bubbles each out-of-order pair one step toward sorted, leaving `[3, 5, 1, 8, 2, 9]`.
...
### Exercise 2.4-1: Playlist Stats (Borrowing Edition)

In the ownership chapter, Exercise 2.3-2 built a playlist manager that had to move
...
> **Key insight**
>
> Compare this to Exercise 2.3-2. Every function borrows the playlist with `&` —
...
### Exercise 2.4-2: In-Place Temperature Converter
...
> **Warning**
>
> If you use `fold` to compute min/max, do not initialize the accumulator with
> an arbitrary constant like `(0.0, 0.0)`. If all values are positive, your
> minimum will be `0.0` — silently wrong. Initialize from the first element
> instead: `data[1..].iter().fold((data[0], data[0]), ...)`, or use
> `f64::INFINITY` and `f64::NEG_INFINITY` as starting values.
...
`celsius_to_fahrenheit` could also be written functionally with `iter_mut()`
and `for_each`:

```rust
fn celsius_to_fahrenheit(temps: &mut [f64]) {
    temps.iter_mut().for_each(|t| *t = *t * 1.8 + 32.0);
}
```

Both forms are equivalent — `for t in temps.iter_mut()` and
`temps.iter_mut().for_each(...)` produce the same code. Pick whichever reads
more clearly in context.

Notice `*t = *t * 1.8 + 32.0` — the `*` dereferences
...
### Exercise 2.4-3: The Word Analyzer
...
- `longest_word` takes a `&str` and returns a `&str` — the longest word. Walk
  ...
  Remember that the last word has no space after it — `find` returns `None` in
  that case, and your loop must still compare that final word.
...
### Exercise 2.4-4: Step Tracker Smoother

A fitness tracker records daily step counts and needs to summarize them, cap
implausible spikes, and smooth noisy days against their neighbors. This exercises
shared slice borrows (`&[u32]`) for read-only summaries, mutable slice borrows
(`&mut [u32]`) for in-place edits, and the index-based pattern for reading
neighboring elements while writing.

**Concepts practiced:** `&[u32]` shared borrows, `&mut [u32]` mutable borrows,
`iter().sum()`, `iter().filter().max().copied()` (from earlier chapters),
`iter_mut()`, the dereference operator `*`, index-based mutation
(`days[i - 1]`, `days[i]`, `days[i + 1]`) when neighboring reads and a write
must coexist, and reusing a value after the borrow ends.

Write four functions:

- `total_steps` takes a `&[u32]` and returns the sum of all days as a `u32`.
- `peak_day` takes a `&[u32]` and returns the `usize` index of the highest day.
  Walk the slice with indices and track the best index so far.
- `cap_outliers` takes a `&mut [u32]` and a `limit`, clamping any day above the
  limit down to the limit, in place. Use `iter_mut()` and `*`.
- `smooth` takes a `&mut [u32]` and replaces each interior day with the average
  of itself and its two neighbors' average. The first and last days are left
  untouched. This needs indices: you read `days[i - 1]` and `days[i + 1]` while
  writing `days[i]`, which a single `&mut` element borrow could not express.

In `main`, create a `Vec<u32>` of seven daily step counts. Print the raw data
and total, then the peak day (bind the index first so the borrow ends and the
`Vec` stays usable), cap outliers, and smooth — printing after each step.

Expected output:

```
Steps:   [3200, 12000, 4100, 9800, 2000, 30000, 5400]
Total:   66500
Peak day: index 5 (30000 steps)
Capped:  [3200, 12000, 4100, 9800, 2000, 15000, 5400]
Smoothed: [3200, 7825, 6456, 7014, 6503, 10475, 5400]
New total: 46873
```

> **Key insight**
>
> `smooth` is the textbook case for indices. The chapter's "many readers or one
> writer" rule means you cannot hold a `&mut` to `days[i]` while also reading
> `days[i - 1]` and `days[i + 1]` through other references. Indexing sidesteps
> this entirely — `days[i]` borrows only for the instant of each access, so the
> read and the write never overlap as live borrows. Note that `smooth` reads the
> already-smoothed previous day, which is fine: the output is fully deterministic
> regardless, because each access happens in a fixed order.

<details>
<summary>Solution</summary>

```rust
fn total_steps(days: &[u32]) -> u32 {
    days.iter().sum()
}

fn peak_day(days: &[u32]) -> usize {
    let mut best = 0;
    for i in 1..days.len() {
        if days[i] > days[best] {
            best = i;
...
fn cap_outliers(days: &mut [u32], limit: u32) {
    for value in days.iter_mut() {
        if *value > limit {
            *value = limit;
        }
    }
...
fn smooth(days: &mut [u32]) {
    for i in 1..days.len() - 1 {
        let neighbor_average = (days[i - 1] + days[i + 1]) / 2;
        days[i] = (days[i] + neighbor_average) / 2;
    }
}

fn main() {
    let mut week = vec![3200, 12000, 4100, 9800, 2000, 30000, 5400];
    println!("Steps:   {week:?}");
    println!("Total:   {}", total_steps(&week));

    let peak = peak_day(&week);
    println!("Peak day: index {peak} ({} steps)", week[peak]);

    cap_outliers(&mut week, 15000);
    println!("Capped:  {week:?}");

    smooth(&mut week);
    println!("Smoothed: {week:?}");
    println!("New total: {}", total_steps(&week));
}
```

`total_steps` and `peak_day` only read, so they take `&[u32]`. The mutating
pair takes `&mut [u32]`: `cap_outliers` uses `iter_mut()` to get a `&mut u32`
per element and `*value` to read and write it. `smooth` cannot use `iter_mut()`
because it needs to look at neighbors — so it indexes, which never creates a
persistent borrow. In `main`, binding `let peak = peak_day(&week)` ends the
shared borrow on that line, leaving `week` free to be mutably borrowed by
`cap_outliers` and `smooth` afterward. The borrows never overlap, so the borrow
checker is satisfied.

</details>

### Exercise 2.4-5: Log Line Parser

Server logs arrive as lines like `[WARN] disk almost full`. This exercise parses
each line into a level and a message — returning borrowed `&str` slices instead
of allocating new strings — then assembles a digest in a separate `String`
buffer. It practices `&str` parameters, returning slices that borrow from the
input, the explicit lifetime annotation required when two references go in and
one comes out, and the shared-to-mutable borrow transition.

**Concepts practiced:** `&str` parameters and returns, returning a borrowed
slice from the input, the explicit lifetime annotation `<'a>` on a function with
two reference parameters (like the chapter's `longer`), `find` plus slicing,
skipping a delimiter byte (`&line[close + 2..]`), let chains (Rust 2024) to
collapse nested `if let`, `&mut String` with `push`/`push_str`, and confirming
the borrowed source is still usable afterward.

Write three functions:

- `level` takes a log line `&str` and a `fallback` `&str`, returning a `&str`.
  If the line contains `[` and `]`, return the text between them; otherwise
  return the fallback. Because two references go in and one comes out, the
  compiler cannot guess which the result borrows from — annotate it `<'a>` so
  both inputs and the output share the lifetime `'a`.
- `message` takes a `&str` and returns the text after `"] "`, or the whole line
  if there is no `]`. Use `find(']')` and slice past the bracket and its
  trailing space with `&line[close + 2..]`.
- `record` takes a `&mut String` buffer and a line `&str`, appending a newline
  separator (unless the buffer is empty), then `level: message`.

In `main`, create an array of three log lines (at least one with no level
bracket). Print the parsed level and message for each, building a digest with
`record`. Print the digest, then the line count to prove the array was only
borrowed.
...
level=INFO, message="no level here, just text"
level=ERROR, message="connection refused"
--- digest ---
WARN: disk almost full
INFO: no level here, just text
...
> **Key insight**
>
> `level` needs the `'a` annotation for the same reason as the chapter's
> `longer`: with two `&str` inputs, the compiler cannot tell whether the returned
> slice borrows from `line` or from `fallback`. Writing `<'a>` on both parameters
> and the return ties them together — the result lives as long as the shorter of
> the two inputs. `message` needs no annotation because it has a single input
> reference, so lifetime elision handles it. Both functions return slices that
> point directly into the caller's data: no allocation, no copying.

<details>
<summary>Solution</summary>

```rust
fn level<'a>(line: &'a str, fallback: &'a str) -> &'a str {
...
fn message(line: &str) -> &str {
    if let Some(close) = line.find(']') {
        // Skip the closing ']' and the single space that follows it.
        &line[close + 2..]
    } else {
        line
    }
...
fn record(buffer: &mut String, line: &str) {
    if !buffer.is_empty() {
        buffer.push('\n');
    }
    buffer.push_str(level(line, "INFO"));
    buffer.push_str(": ");
    buffer.push_str(message(line));
}
...
fn main() {
    ...
    for line in &lines {
        println!("level={}, message={:?}", level(line, "INFO"), message(line));
        record(&mut digest, line);
    }

    println!("--- digest ---");
    println!("{digest}");

    // The source array was only borrowed — still available.
    println!("processed {} lines", lines.len());
}
```

`level` and `message` borrow from `line` and never allocate — the returned
`&str` is a window into the original text. The let chain in `level`
(`if let Some(open) = ... && let Some(close) = ...`) collapses what would
otherwise be two nested `if let`s into one condition, and falls through to the
`fallback` when either bracket is missing. `record` borrows the `String`
mutably with `&mut String` and the line immutably with `&str` — two different
values, so no borrow conflict. After the loop, `lines` is still fully usable
because every call only borrowed it.
