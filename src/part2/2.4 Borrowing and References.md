...
while you are looking at it. This promise is what makes shared references safe to
hand out to multiple parts of your code simultaneously.
...
solution is to return the owned value instead:
...
The read (`iter().max()`) completes before the write (`&mut scores` loop begins).
