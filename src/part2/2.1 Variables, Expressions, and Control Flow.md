...
fn main() {
    ...
    println!("{:?}", a.total_cmp(&b));
    println!("{:?}", nan.total_cmp(&a));
    println!("{:?}", nan.total_cmp(&nan));
}
```
...
Unlike `==`, which says `NaN != NaN`, `total_cmp` treats NaN values as equal
to each other and greater than every finite number. This is the production-grade
way to compare floats when you need a definitive answer — you will use it for
sorting collections and finding minimum and maximum values.
