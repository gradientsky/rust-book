...
others — but they require concepts you will learn later in the book. For now,
the key takeaway is: if you need a named, fixed value, use `const`. If it must
live at a single address, use `static`. If you need it to change, you will
learn the safe patterns for that in Part 5 and Part 6.
...
need a collection that can grow, you will use `Vec<T>` — which is covered in
a later part of the book. For now, arrays are perfect for small, fixed
collections.
