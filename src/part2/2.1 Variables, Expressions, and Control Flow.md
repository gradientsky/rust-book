...
Rust will not implicitly convert between `f32` and `f64`. If a function returns
`f32` but you collect results into a `Vec<f64>`, the compiler will reject it.
Either keep all your types consistent or convert explicitly with `as f64` at
the boundary. Float literals must also look like floats — write `2.0`, not `2`,
when the context requires a floating-point value:
...
### Try It: Bindings and Numeric Types

These short exercises practice `let` bindings, shadowing, type annotations, the turbofish, and the explicit overflow methods, each solvable in a single `main`.

#### Exercise 2.1-A1: Trim and Parse with Shadowing

A program reads a raw measurement as text with surrounding whitespace and needs it as an `i32`. Start with the literal `"  98  "` bound to `raw`. Use shadowing to rebind `raw` to its trimmed `&str`, then bind `reading: i32` by parsing it with `.parse()` and an annotation. Separately, compute `doubled` by parsing the same trimmed string again — but this time with the turbofish `parse::<i32>()` in the middle of an expression that multiplies by 2.

This practices shadowing (changing a value while keeping the name), the `: i32` annotation on a `let`, and the `::<i32>` turbofish when there is no binding to annotate.
...
fn main() {
    let raw = "  98  ";
    let raw = raw.trim();
    let reading: i32 = raw.parse().expect("not a number");

    let doubled = raw.parse::<i32>().expect("not a number") * 2;
...
#### Exercise 2.1-A2: Comparing Overflow Strategies

A running balance is stored as a `u32`. Start with `balance: u32 = 4_000_000_000` and a `deposit: u32 = 500_000_000` — their sum exceeds `u32::MAX` (4,294,967,295), so a plain `+` would panic in debug builds and silently wrap in release builds. Print the result of three explicit methods on `balance`: `checked_add` (returns `Option`, use `{:?}`), `saturating_add` (clamps to the maximum), and `wrapping_add` (two's complement wrap).

This practices the named overflow methods that make boundary behavior explicit instead of relying on build mode. Note the underscores in the literals are just visual separators.

Expected output:

```
checked:    None
saturating: 4294967295
wrapping:   205032704
```
...
```rust
fn main() {
    let balance: u32 = 4_000_000_000;
    let deposit: u32 = 500_000_000;

    println!("checked:    {:?}", balance.checked_add(deposit));
    println!("saturating: {}", balance.saturating_add(deposit));
    println!("wrapping:   {}", balance.wrapping_add(deposit));
}
```
...
When classifying a value into several categories, `match` with guards is often
cleaner than a chain of `if` / `else if`:
...
Each arm binds the value to `t` and tests it with a _guard_ (`if t <= 0`).
The `_` arm catches everything else. You will see `match` in much more detail
in later chapters — for now, know that it is an expression just like `if` and
works well for multi-way classification.
...
time through the loop. Notice that this pattern requires `mut` — the loop body
must modify `n` each time. Later chapters introduce iterator methods like
`fold` that can accumulate a result without mutable variables, letting you lean
further into Rust's immutability-by-default philosophy.
...
> **Tip**
>
> `for x in array` gives you owned values (e.g., `f64`). `for x in &array`
> or `array.iter()` gives references (e.g., `&f64`). If the compiler complains
> about a type mismatch like `expected &f64, found f64` in a comparison, you
> likely need to dereference with `*x` or iterate without `&`.
...
### Try It: Expressions and Control Flow

These short exercises practice `if` as an expression, `loop` with a value-carrying `break`, and `for` over ranges, each solvable in a single `main`.

#### Exercise 2.1-B1: Grade Labels with an If Expression

Given the array of exam scores `[95, 82, 74, 60, 38]`, loop over each one and bind a `grade` letter using an `if`/`else if`/`else` _expression_ — no mutable variable, each branch returns a `&str`. Use `>= 90` for `"A"`, `>= 80` for `"B"`, `>= 70` for `"C"`, `>= 60` for `"D"`, and `"F"` otherwise. Print each score right-aligned in a 3-character field followed by its grade.

This practices `if` as an expression whose value binds directly to a variable, the same idea you have been using since the bindings section.
...
#### Exercise 2.1-B2: Loop That Returns a Value

Find the smallest positive integer `n` whose square is at least `100`, then count down from it. Use a `loop` that increments a counter and `break`s _with the value_ once `n * n >= 100`, binding the result to `count`. Then use a `for` loop over `(1..=count).rev()` to print each `n` and its square, counting down.

This practices `loop` as an expression — `break count;` makes the whole loop evaluate to a value — and the inclusive range `..=` combined with `.rev()` to iterate downward.

Expected output:

```
smallest n with n*n >= 100: 10
10 squared = 100
9 squared = 81
8 squared = 64
7 squared = 49
...
fn main() {
    ...
    println!("smallest n with n*n >= {target}: {count}");
...
> **Tip**
>
> `vec![1, 2, 3]` is a macro that creates a `Vec` and populates it in one
> step. It allocates exactly the right capacity upfront. You can also repeat a
> value: `vec![0; 10]` creates ten zeros. When you do not know the elements
> ahead of time, use `Vec::new()` to start empty and `push` items in a loop,
> or `Vec::with_capacity(n)` to pre-allocate space when you know roughly how
...
### Try It: Compound Types and Let Chains

These short exercises practice tuples, arrays, `Vec`, the `as` conversion, and let chains, each solvable in a single `main`.

#### Exercise 2.1-C1: Averaging Tuple Readings

Given the array of `(id, value)` readings `[(3, 14.0), (1, 9.5), (4, 22.0), (2, 7.5)]`, loop over it and destructure each tuple into `(id, value)` in the `for` pattern. Print each reading, accumulating the values into a running `total`. After the loop, compute the average by dividing `total` by the array length — remember `.len()` returns a `usize`, so convert it with `as f64`. Print the average to two decimal places.

This practices tuple destructuring in a `for` loop and the `total / len() as f64` averaging pattern.
...
```rust
fn main() {
    let readings = [(3, 14.0), (1, 9.5), (4, 22.0), (2, 7.5)];

    let mut total = 0.0;
    for (id, value) in readings {
        println!("sensor {id}: {value:.1}");
        total += value;
    }

    let average = total / readings.len() as f64;
    println!("average over {} sensors: {average:.2}", readings.len());
}
```
...
#### Exercise 2.1-C2: Collecting Valid Bytes with a Let Chain

A program receives a list of text inputs and must collect only those that name a valid byte value (0 to 255). Given `["7", "200", "-4", "55", "30x", "180"]` and a `max` of `255`, loop over each input and use a _let chain_ to parse it as an `i32` _and_ check it falls in `0..=max`. On success, narrow it to a `u8` with `as u8` and push it into a `Vec<u8>`; otherwise print a `rejected:` line. Finish by printing the accepted vector and how many inputs were valid.

This practices building a `Vec` with `push`, the `if let ... && ... && ...` let chain from Rust 2024, and the `as u8` narrowing once the value is known to be in range.
...
rejected: -4
rejected: 30x
accepted: [7, 200, 55, 180]
4 of 6 inputs were valid bytes
```
...
fn main() {
    ...
    println!("accepted: {bytes:?}");
    println!("{} of {} inputs were valid bytes", bytes.len(), inputs.len());
}
```
...
- **`mut` is not always necessary.** Iterator methods like `fold` can
  replace mutable accumulators entirely — you will see this in later chapters.
  When you spot a `let mut` followed by a loop that updates it, ask whether a
  functional approach could eliminate the mutation.
- **Let chains are the Rust 2024 way** to combine pattern matching and boolean
  conditions without nested `if` statements.

## Exercises

These micro-projects let you practice the chapter's core ideas hands-on. Each
one combines several concepts — bindings, expressions, control flow, and numeric
methods — into a small, self-contained program. Try to solve them before reading
the solution.

### Exercise 2.1-1: Temperature Report

**Goal.** Convert a fixed array of Celsius temperatures to Fahrenheit, classify
each reading, and count how many exceed a threshold.

**Your task:**

1. Create an array of `f64` temperatures in Celsius:
   `[0.0, 15.0, 25.0, 37.0, -5.0, 100.0]`.
2. Loop over the array and convert each value to Fahrenheit using the formula
   `F = C × 9 / 5 + 32`.
3. Classify each temperature using an `if` expression that returns a `&str`
   label: `"freezing"` (≤ 0), `"cold"` (≤ 15), `"mild"` (≤ 30), or `"hot"`
   (above 30).
4. Count how many readings exceed 20 °C.
5. Print each line with right-aligned, one-decimal formatting and print the
   final count.

> **Key insight.** The `if` expression _returns_ the label — no mutable
> variable needed. The `as` keyword is not required here because the
> temperatures start as `f64`, but the format specifiers `{:>6.1}` control
> alignment and precision in the output.
>
> **Stretch.** Define a `celsius_to_fahrenheit` function and use
> `.iter().map(|&x| celsius_to_fahrenheit(x)).collect()` to build a `Vec` of
> Fahrenheit values in one expression. Then use `.zip()` to iterate both
> arrays together, and try `match` with guards for the classification.
>
> **Functional tips.** `.iter()` yields references (`&f64`), so write
> `|&x|` in the closure to destructure them into owned values. You can pass
> a function directly to `.map()` — e.g., `.copied().map(celsius_to_fahrenheit)` —
> instead of wrapping it in a closure. `.zip()` pairs up two iterators
> element by element: `for (c, f) in temps_c.iter().zip(temps_f.iter())`.
...
----------------------------------
  ...
  25.0 °C =   77.0 °F  [mild]
  37.0 °C =   98.6 °F  [hot]
  -5.0 °C =   23.0 °F  [freezing]
 100.0 °C =  212.0 °F  [hot]
----------------------------------
3 readings above 20 °C
```
...
```rust
fn main() {
    let temperatures_c = [0.0_f64, 15.0, 25.0, 37.0, -5.0, 100.0];
    ...
    for temp_c in temperatures_c {
        ...
        println!("{temp_c:>6.1} °C = {temp_f:>6.1} °F  [{label}]");
    }

    println!("{:-<34}", "");
    println!("{above_count} readings above {threshold} °C");
}
```

The `if` expression assigns `label` without any mutation — each branch returns a
`&str` of the same type. The `for` loop iterates directly over the array values.
The format specifier `{:>6.1}` right-aligns each number in a 6-character field
with one decimal place, keeping the output columns neat.

</details>

### Exercise 2.1-2: Saturating Scoreboard

**Goal.** Track a game score stored as a `u8` (0–255) that must never overflow
or underflow, using saturating arithmetic.

**Your task:**

1. Start a mutable `u8` score at `50`.
2. Apply these changes in order, printing the score after each one:
   - Add 220 using saturating arithmetic (should clamp at 255, not panic or wrap)
   - Subtract 200
   - Add 30
   - Subtract 100 using saturating arithmetic (should clamp at 0)
3. After all changes, print the final score and a status message: `"still in
   the game"` if the score is above 0, or `"game over"` otherwise. Use an `if`
   expression to choose the message.

> **Key insight.** Normal `+` and `-` would panic in debug mode or wrap in
> release mode. `saturating_add` and `saturating_sub` make the boundary
> behavior explicit — the intent is readable directly in the code.
>
> **Stretch.** Try replacing the final `if` expression with a `match` that
> also prints `"perfect score"` when the score is exactly 255.
>
> **Advanced stretch.** Define an enum with `Add(u8)` and `Sub(u8)` variants
> to represent score operations. Store the operations in an array and use
> `.iter().fold(score, ...)` to apply them all — no `mut` needed. Use `match`
> inside the fold to pick `saturating_add` or `saturating_sub` based on the
> variant.
...
fn main() {
    ...
    // Big bonus — should clamp at 255, not overflow
    score = score.saturating_add(220);
    println!("+220:   {score}");

    // Penalty — should clamp at 0, not underflow
    ...
    let status = if score > 0 { "still in the game" } else { "game over" };
    println!("final:  {score} ({status})");
}
```

Every arithmetic operation uses a saturating method, so the score clamps at 0 or
255 instead of wrapping or panicking. The final `if` expression returns the
status string directly — no mutable variable or `match` needed.

</details>

### Exercise 2.1-3: Input Validator

**Goal.** Parse a list of string inputs, accept only those that are valid
positive integers within a range, and sort the results into accepted and
rejected lists — using let chains for the combined check.

**Your task:**

1. Create an array of string slices: `["42", "hello", "-3", "100", "7x", "55"]`.
2. Set a maximum value of `100`.
3. Loop over the inputs. For each one, use a _let chain_ to parse it as an
   `i32` _and_ check that the result is between 1 and `max` (inclusive).
4. Push accepted numbers into a `Vec<i32>` and rejected strings into a
   `Vec<&str>`.
5. Print both lists and how many inputs were valid.

> **Key insight.** The let chain `if let Ok(n) = input.parse::<i32>() && n > 0
> && n <= max` combines parsing with `parse::<i32>()` and validation in one
> flat condition. If the parse fails, the range check never runs.
>
> **Common mistake.** You might try `Vec<i32>()` to create an empty vector —
> Rust parses `<` and `>` as comparison operators, not generic parameters.
> Write `Vec::<i32>::new()` (turbofish) or annotate the binding:
> `let v: Vec<i32> = Vec::new();`. When the compiler can infer the type from
> a later `.push()` call, plain `Vec::new()` is enough.
>
> **Stretch.** Rewrite the loop as a single `.fold()` call that accumulates
> a `(Vec<i32>, Vec<&str>)` tuple. Use `match` with a guard inside the fold
> instead of a let chain. This eliminates the top-level `let mut` — mutation
> is confined inside the fold closure.
>
> **Functional tips.** A `fold` closure must return the accumulator — if the
> last expression is a `match` whose arms call `.push()` (which returns
> `()`), the compiler will complain that it expected your accumulator type
> but got `()`. Add `(acc, rej)` as the final line after the `match`.
> `partition` is simpler when both sides have the same type, but here the
> accepted and rejected lists differ (`i32` vs `&str`), so `fold` is the
> right tool.
...
```rust
fn main() {
    let inputs = ["42", "hello", "-3", "100", "7x", "55"];
    let max = 100;
    ...
    println!("{} of {} inputs were valid", accepted.len(), inputs.len());
}
```

The let chain does three things in one condition: parse the string, check that
the number is positive, and check that it does not exceed the maximum. `"-3"`
parses successfully as an `i32` but fails the `n > 0` check, so it lands in the
rejected list alongside the unparseable strings. Without let chains, this logic
would require nested `if let` and `if` blocks.

</details>

### Exercise 2.1-4: Pixel Brightness Adjuster

**Goal.** Brighten a small set of RGB pixels by a fixed amount, keeping every
color channel inside the valid `u8` range and reporting the result in
hexadecimal.

**Concepts practiced:** arrays and tuples, destructuring, `for` over array
values, `as` widening (`u8` → `i32`) and narrowing (`i32` → `u8`), the `clamp`
method, `mut` accumulator, format specifiers (`{:>3}`, `{:02x}`, `{:-<40}`).

**Your task:**

1. Create an array of four RGB pixels typed `[(u8, u8, u8); 4]`:
   `(200, 180, 50)`, `(10, 20, 30)`, `(255, 128, 0)`, `(60, 90, 120)`.
2. Set a brightness `adjustment: i32 = 80`.
3. For each pixel, widen each channel to `i32`, add the adjustment, and
   `clamp` the result into `0..=255` before narrowing back to `u8`. Widening
   first means the addition can never overflow a `u8`.
4. Count how many individual channels had to be clipped (the clamped value
   differs from the raw sum).
5. Print the three adjusted channels as a `#rrggbb` hex color by formatting each
   one with `{:02x}` (two-digit, zero-padded, lowercase hex), the same
   per-channel hex you used in the color exercise back in the first chapter.
6. Print each row with right-aligned channel values, then the total clipped
   channel count.

**Expected output:**

```
Brightness adjust: 80
----------------------------------------
original       adjusted       hex
(200,180, 50)  (255,255,130)  #ffff82
( 10, 20, 30)  ( 90,100,110)  #5a646e
(255,128,  0)  (255,208, 80)  #ffd050
( 60, 90,120)  (140,170,200)  #8caac8
----------------------------------------
3 channels clipped to range
```
...
```rust
fn main() {
    // Each pixel is an (R, G, B) triple of u8 channels (0–255).
    let pixels: [(u8, u8, u8); 4] = [
        (200, 180, 50),
        (10, 20, 30),
        (255, 128, 0),
        (60, 90, 120),
    ...
    // Brightness adjustment applied to every channel.
    // Positive values brighten, negative values darken.
    let adjustment: i32 = 80;

    println!("Brightness adjust: {adjustment}");
    println!("{:-<40}", "");
    println!("{:<13}  {:<13}  hex", "original", "adjusted");

    let mut clipped_channels = 0u32;

    for (r, g, b) in pixels {
        // Widen to i32 so the math cannot overflow a u8, add the adjustment,
        // clamp back into 0–255, and narrow to u8 once the value is in range.
        let nr = (r as i32 + adjustment).clamp(0, 255);
        let ng = (g as i32 + adjustment).clamp(0, 255);
        let nb = (b as i32 + adjustment).clamp(0, 255);

        if nr != r as i32 + adjustment {
            clipped_channels += 1;
        }
        if ng != g as i32 + adjustment {
            clipped_channels += 1;
        }
        if nb != b as i32 + adjustment {
            clipped_channels += 1;
        }

        let (ar, ag, ab) = (nr as u8, ng as u8, nb as u8);

        // Print the color as #rrggbb, formatting each channel as two-digit
        // zero-padded lowercase hex — the same per-channel hex you used in
        // the color exercise back in the first chapter.
        println!(
            "({r:>3},{g:>3},{b:>3})  ({ar:>3},{ag:>3},{ab:>3})  #{ar:02x}{ag:02x}{ab:02x}"
        );
    }

    println!("{:-<40}", "");
    println!("{clipped_channels} channels clipped to range");
}
```

Widening each channel to `i32` before adding keeps the sum from overflowing a
`u8`, and `clamp(0, 255)` guarantees the value fits before the narrowing
`as u8` cast — no silent truncation. Pinning the array type to `[(u8, u8, u8); 4]`
makes every channel a `u8`, so the `as i32` casts are genuine widenings rather
than no-ops. Formatting each adjusted channel with `{:02x}` prints it as a
two-digit, zero-padded, lowercase hex pair, so the three together read as a
`#rrggbb` color.

</details>

### Exercise 2.1-5: Binary Search With Safe Midpoint

**Goal.** Search a sorted lookup table with binary search, using `midpoint` to
avoid the classic index-overflow bug, and report how many comparison steps each
lookup took.

**Concepts practiced:** sorted arrays, `usize` indices, `loop` as an expression
with `break value`, the `usize::midpoint` method, `if`/`else if`/`else`, `mut`
loop state, ranges via `.len()`, format specifiers (`{:>5}`, `{:-<38}`).

**Your task:**

1. Create a sorted array `stations: [u32; 11]` of IDs:
   `[3, 17, 42, 56, 91, 128, 256, 512, 1024, 2048, 4096]`.
2. Create an array of targets to look up:
   `[42, 4096, 7, 1024, 3, 100]`.
3. For each target, run a binary search over `stations`:
   - Track `low` and `high` as `usize` window bounds and a `steps` counter.
   - Use a `loop` whose `break` carries the matching index out on a hit, or
     `stations.len()` — a deliberately out-of-range index — when the window
     collapses without a match.
   - Compute the midpoint with `low.midpoint(high)` instead of
     `(low + high) / 2`, which would risk overflow on large indices.
4. Use an `if`/`else` on the result to print whether the target was found (and
   at which index) along with the step count: an index below `stations.len()`
   means found, the sentinel `stations.len()` means not found.
5. Print how many of the targets were found.

> **Key insight.** `low.midpoint(high)` computes the average as if in a wider
> type, so it never overflows even when `low + high` would. Because `loop` is an
> expression, every `break` hands a value back to the `let found_at = loop { ...
> };` binding — the matching index on a hit, or an out-of-range sentinel index
> on a miss — so no separate mutable flag is needed.

**Expected output:**

```
Binary Search (11 entries)
--------------------------------------
   42: found at index 2 in 2 step(s)
 4096: found at index 10 in 4 step(s)
    7: not found        in 4 step(s)
 1024: found at index 8 in 2 step(s)
    3: found at index 0 in 3 step(s)
  100: not found        in 4 step(s)
--------------------------------------
4 of 6 targets found
```
...
```rust
fn main() {
    // A sorted lookup table of station IDs (must stay sorted for binary search).
    let stations: [u32; 11] = [
        3, 17, 42, 56, 91, 128, 256, 512, 1024, 2048, 4096,
    ];

    let targets = [42u32, 4096, 7, 1024, 3, 100];

    println!("Binary Search ({} entries)", stations.len());
    println!("{:-<38}", "");
    ...
    for target in targets {
        let mut low = 0usize;
        let mut high = stations.len() - 1;
        let mut steps = 0u32;

        // `break` carries a value out of the loop: the matching index on a
        // hit, or `stations.len()` (a deliberately out-of-range index) to
        // signal "not found" when the search window collapses.
        let found_at = loop {
            steps += 1;
            // midpoint avoids the classic (low + high) overflow bug.
            let mid = low.midpoint(high);

            if stations[mid] == target {
                break mid;
            } else if stations[mid] < target {
                if mid == high {
                    break stations.len();
                }
                low = mid + 1;
        ...
        if found_at < stations.len() {
            found_count += 1;
            println!("{target:>5}: found at index {found_at} in {steps} step(s)");
        } else {
            println!("{target:>5}: not found        in {steps} step(s)");
        }
    }

    println!("{:-<38}", "");
    println!("{found_count} of {} targets found", targets.len());
}
```

Each iteration narrows the `[low, high]` window by half. `low.midpoint(high)`
keeps the index calculation correct even if `low + high` would exceed
`usize::MAX` — the bug that hid in production binary searches for years. The
`break mid` / `break stations.len()` pattern makes the `loop` evaluate to a
`usize`: a real index on a hit, or an out-of-range sentinel on a miss. The
`if found_at < stations.len()` check then turns that into a readable report
line. The `mid == low` and `mid == high` guards stop the search when the window
can no longer shrink, preventing an `index - 1` underflow on a `usize`.

</details>

### Exercise 2.1-6: Sensor Statistics

**Goal.** Clean a batch of floating-point sensor readings by dropping corrupt
values, then compute summary statistics with a total float ordering.

**Concepts practiced:** `f64` special values (`NAN`, `INFINITY`), the
`is_finite` method, `Vec` with `push`, `f64::total_cmp` for sorting,
`usize::midpoint` for the median index, `is_multiple_of` for an even-length
check, `round_ties_even` for unbiased rounding, `as f64` conversion, format
specifiers (`{:>6.2}`).

**Your task:**

1. Create an array `raw` of nine `f64` readings that includes two `f64::NAN`
   values and one `f64::INFINITY` mixed in with ordinary numbers:
   `[21.5, NaN, 19.0, INFINITY, 23.2, 18.7, 20.0, NaN, 24.6]`.
2. Build a `Vec<f64>` of only the finite readings using `is_finite`, counting
   how many values you discard.
3. Print the raw, discarded, and valid counts.
4. Sort the clean readings with `sort_by(f64::total_cmp)` — ordinary `<`
   comparison cannot define a total order once NaN is involved, and `total_cmp`
   gives sorting code a stable rule.
5. Compute the mean (sum divided by the count as `f64`), and the median: use
   `0.midpoint(count - 1)` to find the middle index, then average the two
   central values when `count.is_multiple_of(2)`.
6. Print the min, max, mean, and median right-aligned to two decimals, then the
   mean rounded with `round_ties_even`.

> **Key insight.** Floats implement `PartialOrd` but not `Ord` because `NaN`
> refuses to compare, so `sort` is unavailable — `sort_by(f64::total_cmp)` is the
> production-grade way to order them. Filtering with `is_finite` first removes
> the NaN and infinity values that would otherwise poison every statistic.
...
```rust
fn main() {
    // Raw readings straight off a flaky sensor: some are corrupt
    // (NaN from a divide-by-zero, inf from a saturated amplifier).
    let raw = [
        21.5_f64,
        f64::NAN,
        19.0,
    ...
    // Keep only the ordinary finite numbers; drop NaN and infinities.
    ...
    println!("readings:  {}", raw.len());
    println!("discarded: {discarded}");
    println!("valid:     {}", clean.len());

    // total_cmp gives floats a total order, so sorting is well defined.
    ...
    // The median index pair uses midpoint to find the middle safely.
    let mid = 0usize.midpoint(count - 1);
    let median = if count.is_multiple_of(2) {
        (clean[mid] + clean[mid + 1]) / 2.0
    } else {
        clean[mid]
    };

    println!("{:-<34}", "");
    println!("min:    {:>6.2}", clean[0]);
    println!("max:    {:>6.2}", clean[count - 1]);
    println!("mean:   {mean:>6.2}");
    println!("median: {median:>6.2}");

    // round_ties_even keeps repeated rounding unbiased.
    let rounded = mean.round_ties_even();
    println!("rounded mean (ties to even): {rounded}");
}
```

`is_finite` rejects both `NaN` and infinity in one check, leaving six clean
values. After `sort_by(f64::total_cmp)` the slice is `[18.7, 19.0, 20.0, 21.5,
23.2, 24.6]`, so `clean[0]` and `clean[count - 1]` are the min and max. With an
even count, `0.midpoint(5)` rounds toward zero to `2`, and the median is the
average of `clean[2]` and `clean[3]` — `(20.0 + 21.5) / 2.0 = 20.75`. The mean
`21.166…` rounds to `21` either way here, but `round_ties_even` is the right
default whenever many values are rounded and summed.
