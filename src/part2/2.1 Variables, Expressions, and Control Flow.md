...
2. Apply these changes in order, printing the score after each one:
   - Add 220 (should clamp at 255, not panic or wrap)
   - Subtract 200
   - Add 30
   - Subtract 100 (should clamp at 0)
...
> && n <= max` combines parsing and validation in one flat condition. If the
> parse fails, the range check never runs.
