...
Here, `a` and `b` must be the _same_ type `T`. Writing
`fn largest(a: impl PartialOrd, b: impl PartialOrd)` would allow `a` and `b`
to be _different_ types — both comparable, but not necessarily to each other.
