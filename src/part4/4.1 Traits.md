...
The `join` function returns `impl fmt::Display` — you can pass the result
directly to `println!`, `format!`, or `write!` without allocating a `String`
first. The lifetime `'a` ensures the returned formatter borrows `items` and
`sep` safely, and the `move` keyword transfers those borrows into the closure.
