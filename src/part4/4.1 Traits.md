...
### Try It: Defining and Implementing Traits

These short exercises practice defining a trait, implementing it for more than
one type with a default method, and writing functions that accept any
implementing type.

#### Exercise 4.1-A1: Animal Sounds

Define an `Animal` trait with two required methods — `name(&self) -> String`
and `sound(&self) -> String` — and a default method
`introduce(&self) -> String` that returns `"{name} says {sound}"`.

Implement the trait for two structs, `Dog` and `Cat`, each holding a `name:
String`. `Dog` uses the default `introduce`; `Cat` overrides it to insert
`"the cat"` before `says`.

In `main`, create one of each and print its introduction. This practices a
required-plus-default trait and overriding the default in one of the two
implementors.
...
impl Animal for Cat {
    ...
    fn introduce(&self) -> String {
        format!("{} the cat says {}", self.name(), self.sound())
    }
}
...
#### Exercise 4.1-A2: Visitor Greeter

Define a `Greet` trait with one required method `greeting(&self) -> String`.
Implement it for two structs, `Formal` and `Casual`, each holding a `name:
String`, with different greeting styles.

Write a free function `welcome(visitor: &impl Greet)` that prints the visitor's
greeting. In `main`, create one of each struct and pass both to `welcome`. This
practices the `&impl Trait` parameter — one function that accepts any type
implementing `Greet`.
...
fn welcome(visitor: &impl Greet) {
    println!("{}", visitor.greeting());
}

fn main() {
    let director = Formal {
        name: String::from("Dr. Vance"),
    };
    let friend = Casual {
...
### Try It: Derive, Default, and Display

These exercises practice deriving common traits, using `Default` with struct
update syntax, and implementing `Display` by hand alongside a derived `Debug`.

#### Exercise 4.1-B1: Game Settings

Define a `GameSettings` struct with three fields: `sound_on: bool`,
`difficulty: u32`, and `player_name: String`. Derive `Debug`, `Clone`,
`PartialEq`, and `Default` in a single `#[derive(...)]` line.

In `main`, create a default value and print it with `{:?}`. Then build a custom
value that overrides `difficulty` and `player_name`, leaving the rest at their
defaults with `..GameSettings::default()`. Clone it, then compare the custom
value against both its clone and the default with `==`.

This practices a four-trait derive working together: `Default` supplies the
baseline, struct update syntax fills the rest, and `Clone` plus `PartialEq` let
you copy and compare.

Expected output:

```
GameSettings { sound_on: false, difficulty: 0, player_name: "" }
GameSettings { sound_on: false, difficulty: 3, player_name: "Ada" }
custom == backup: true
...
fn main() {
    ...
    let backup = custom.clone();
    println!("{custom:?}");
    println!("custom == backup: {}", custom == backup);
    println!("custom == defaults: {}", custom == defaults);
}
```
...
#### Exercise 4.1-B2: Compass Display

Define a `Direction` enum with variants `North`, `East`, `South`, and `West`,
and derive `Debug`. Then implement `Display` by hand so each variant prints as
`"heading N"`, `"heading E"`, and so on.

In `main`, loop over an array of all four directions and print each one with
both `{}` (Display) and `{:?}` (Debug) to show the contrast. Then call
`.to_string()` on a direction to confirm the blanket `ToString` implementation
comes for free with `Display`.

This practices the split between the two formatting traits: `Debug` is derived
for developers, `Display` is hand-written for users.

Expected output:

```
display: heading N | debug: North
display: heading E | debug: East
display: heading S | debug: South
display: heading W | debug: West
stored: heading W
```
...
impl fmt::Display for Direction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Direction::North => write!(f, "heading N"),
            Direction::East => write!(f, "heading E"),
            Direction::South => write!(f, "heading S"),
            Direction::West => write!(f, "heading W"),
        }
    }
}
...
fn main() {
    ...
    for heading in &route {
        println!("display: {heading} | debug: {heading:?}");
    }

    let label: String = Direction::West.to_string();
    println!("stored: {label}");
}
```
...
### Try It: Conversion Traits

These exercises practice the standard conversion traits: `From`/`Into` for
infallible conversions and `FromStr` for parsing strings into your own types.

#### Exercise 4.1-C1: User Id Newtype

Define a `UserId(u64)` newtype struct and implement `From<u64> for UserId`.

In `main`, build one `UserId` with the explicit `UserId::from(...)` and another
with `.into()` (the mirror you get for free). Then write a free function
`show_user(id: impl Into<UserId>)` that converts its argument with `.into()`
and prints the inner number, and call it with a raw `u64`.

This practices implementing `From`, using `Into`, and the `impl Into<_>`
parameter that lets a function accept the raw value directly.
...
fn show_user(id: impl Into<UserId>) {
    let id = id.into();
    println!("user #{}", id.0);
}
...
fn main() {
    ...
    println!("admin is user #{}", admin.0);

    // Into is the mirror — provided for free
    let guest: UserId = 1000.into();
    println!("guest is user #{}", guest.0);

    // A function bounded by Into accepts the raw value directly
    show_user(42);
}
```
...
#### Exercise 4.1-C2: Parse a Direction

Define a `Direction` enum with variants `North`, `East`, `South`, and `West`,
and derive `Debug` and `PartialEq`. Implement `FromStr` so it accepts both the
full name and a single-letter abbreviation, case-insensitively (`"North"`,
`"n"`, `"N"` all parse to `North`). Use `type Err = String` and return a
descriptive error for anything else.

In `main`, parse one direction with `.parse()`, one with
`Direction::from_str(...)`, and show an error case. Note the pitfall: a bare
`"42".parse()` cannot infer the target type — annotate the binding (as in
`let result: Result<Direction, _>`) or use the turbofish so Rust knows which
`FromStr` implementation to call.
...
fn main() {
    ...
    let short = Direction::from_str("w").unwrap();
    println!("{short:?}");

    let result: Result<Direction, _> = "up".parse();
    println!("{result:?}");
}
```
...
### Exercise 4.1-1: Weight Converter
...
### Exercise 4.1-4: RGB Color Mixer

Build a color system that parses colors from hex strings, blends them with the
`+` operator, and reports their brightness through a custom trait. This
exercises `FromStr`, `Display`, `From` from a tuple, `Add` operator
overloading, a custom trait with a default method, and an `impl Trait`
parameter.

**Concepts practiced:** custom trait with supertrait `Display` and a default method, `Display`, `From<(u8, u8, u8)>`, `Add` operator overloading, `FromStr`, `impl Trait` parameters, `.to_string()` from `Display`.

Build these pieces:

- A `Color` struct with `r`, `g`, `b` fields of type `u8`. Derive `Debug`,
  `Clone`, `Copy`, `PartialEq`, `Eq`, and add a `new(r, g, b)` associated
  function.
- `Display` for `Color` producing an uppercase hex string like `"#FF0000"`
  (use `{:02X}` for each channel).
- `From<(u8, u8, u8)> for Color` so a tuple can become a color.
- `Add` for `Color` that blends two colors by saturating-adding each channel
  (so values cap at 255 instead of wrapping).
- `FromStr` for `Color` that parses `"#RRGGBB"` (with or without the leading
  `#`) using `u8::from_str_radix(slice, 16)`. Return descriptive errors for the
  wrong length or invalid hex digits.
- A `Channels` trait with supertrait `Display`, a required
  `luminance(&self) -> f64` method, and a default `brightness(&self) -> &str`
  method that returns `"light"` when luminance is at least `128.0`, else
  `"dark"`.
- Implement `Channels` for `Color` using the Rec. 601 luma formula
  `0.299*r + 0.587*g + 0.114*b`.
- A free function `describe(color: &impl Channels)` that prints the color, its
  luminance to one decimal place, and its brightness label.
- In `main`, build one color by parsing hex, one from a tuple, and one with
  `new`; describe all three; blend with `+`; demonstrate `.to_string()`; and
  show an error case for a too-short hex string.

Expected output:

```
#FF0000 -> luminance 76.2 (dark)
#00FF00 -> luminance 149.7 (light)
#0000FF -> luminance 29.1 (dark)
red + green = #FFFF00
red + green + blue = #FFFFFF
label: #0000FF
parsing '#12345': Err("expected 6 hex digits, got '12345'")
```
...
impl FromStr for Color {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let parse = |slice: &str| -> Result<u8, String> {
            u8::from_str_radix(slice, 16).map_err(|_| format!("invalid hex: '{slice}'"))
        };
        let r = parse(&hex[0..2])?;
        let g = parse(&hex[2..4])?;
        let b = parse(&hex[4..6])?;
...
fn describe(color: &impl Channels) {
    println!("{color} -> luminance {:.1} ({})", color.luminance(), color.brightness());
}
...
fn main() {
    ...
    println!("red + green + blue = {white}");

    let label = blue.to_string();
    println!("label: {label}");

    let bad: Result<Color, _> = "#12345".parse();
    println!("parsing '#12345': {bad:?}");
}
```

</details>

### Exercise 4.1-5: Money Ledger

Model money as integer cents and build a ledger that parses amounts from
strings, adds them with the `+` operator, and computes a running balance. This
exercises a newtype-style struct with `Display`, `Add`, `FromStr`, an enum with
its own `Display`, and slice-based accumulation — keeping all arithmetic in
integer cents to avoid floating-point rounding errors.

**Concepts practiced:** `Display` (including formatting negative values), `Add` operator overloading, `FromStr` with `split_once` and `?`, an enum with a `Display` impl and a method, slice parameters, and `.parse()` driven by `FromStr`.

Build these pieces:

- A `Money` struct with a single `cents: i64` field. Derive `Debug`, `Clone`,
  `Copy`, `PartialEq`, `Eq`, and add a `from_cents(cents)` associated function.
- `Display` for `Money` that prints values like `"$100.00"` and `"-$4.45"`.
  Handle the sign explicitly and use `cents / 100` for dollars and
  `cents % 100` for the remaining cents (formatted with `{:02}`).
- `Add` for `Money` that adds the cents of both values.
- `FromStr` for `Money` that parses `"$DOLLARS.CC"`. Strip the leading `$`
  (error if missing), split on `.` with `split_once` (error if missing), parse
  the dollars, and require exactly two cent digits. Return descriptive `String`
  errors at each failure.
- An `Entry` enum with variants `Deposit(Money)` and `Withdrawal(Money)`, a
  `signed(&self) -> Money` method (deposits stay positive, withdrawals become
  negative), and a `Display` impl printing `"deposit  $5.50"` /
  `"withdraw $80.00"`.
- A free function `balance(entries: &[Entry]) -> Money` that sums every entry's
  signed amount using `+`.
- In `main`, build a `Vec<Entry>` from parsed amounts, print the ledger, print
  the balance, add two `Money` values directly, and show an error case for an
  amount missing its `$`.
...
balance: -$4.45
rent + utilities: $1284.99

parsing '100.00': Err("missing '$': '100.00'")
```
...
impl FromStr for Money {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let body = s.trim().strip_prefix('$').ok_or(format!("missing '$': '{s}'"))?;
        let (dollars, cents) = body.split_once('.').ok_or(format!("missing '.': '{body}'"))?;
        let dollars: i64 = dollars
            .parse()
            .map_err(|_| format!("invalid dollars: '{dollars}'"))?;
        if cents.len() != 2 {
            return Err(format!("cents must have 2 digits: '{cents}'"));
        }
        let cents: i64 = cents.parse().map_err(|_| format!("invalid cents: '{cents}'"))?;
        Ok(Money {
...
impl Entry {
    fn signed(&self) -> Money {
        match self {
            Entry::Deposit(m) => *m,
            Entry::Withdrawal(m) => Money::from_cents(-m.cents),
        }
    }
}
...
impl fmt::Display for Entry {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Entry::Deposit(m) => write!(f, "deposit  {m}"),
            Entry::Withdrawal(m) => write!(f, "withdraw {m}"),
        }
    }
}

fn balance(entries: &[Entry]) -> Money {
    let mut total = Money::from_cents(0);
    for entry in entries {
        total = total + entry.signed();
    }
    total
}
...
fn main() {
    let entries = vec![
        Entry::Deposit("$100.00".parse().unwrap()),
        Entry::Withdrawal("$29.95".parse().unwrap()),
        Entry::Deposit("$5.50".parse().unwrap()),
        Entry::Withdrawal("$80.00".parse().unwrap()),
    ];
    ...
    let total = balance(&entries);
    println!("\nbalance: {total}");

    let rent: Money = "$1200.00".parse().unwrap();
    let utilities = Money::from_cents(8499);
    println!("rent + utilities: {}", rent + utilities);

    let bad: Result<Money, _> = "100.00".parse();
    println!("\nparsing '100.00': {bad:?}");
}
```
...
### Exercise 4.1-6: Lap Timer

Build a duration type that converts from raw seconds, formats as `HH:MM:SS`,
adds with `+`, and parses flexible colon-separated strings — then label each
duration through an `AsRef<str>`-based helper. This exercises `From<u64>`,
`Display`, `Add`, `FromStr`, a custom trait whose default method takes another
`impl Trait`, `AsRef<str>`, and `impl Into<String>`.

**Concepts practiced:** `From<u64>`, `Display` with width-padded numeric formatting, `Add` operator overloading, `FromStr` parsing 1-3 colon-separated fields, a custom trait with a supertrait and a default method, `AsRef<str>`, a generic `impl AsRef<str>` parameter, and an `impl Into<String>` constructor.

Build these pieces:

- A `Duration` struct with a single `seconds: u64` field. Derive `Debug`,
  `Clone`, `Copy`, `PartialEq`, `Eq`, and add a `new(seconds)` associated
  function.
- `Display` for `Duration` printing `HH:MM:SS` (each field zero-padded to two
  digits with `{:02}`), splitting seconds into hours, minutes, and seconds.
- `From<u64> for Duration` so a raw second count becomes a duration.
- `Add` for `Duration` that adds the seconds of both values.
- `FromStr` for `Duration` that parses 1 to 3 colon-separated fields. Fold each
  field into the running total with `total = total * 60 + value`, so `"90"`,
  `"01:05"`, and `"1:00:00"` all work. Error on an invalid number or on more
  than three fields.
- A `Timed` trait with supertrait `Display`, a required
  `total_seconds(&self) -> u64` method, and a default
  `is_longer_than(&self, other: &impl Timed) -> bool` method comparing the two
  totals. Implement it for `Duration`.
- A `Lap` struct with `name: String` and `elapsed: Duration`, a
  `new(name: impl Into<String>, elapsed: Duration)` constructor, and an
  `AsRef<str>` impl returning the name.
- A free function `widest_name(laps: &[impl AsRef<str>]) -> usize` returning the
  length of the longest name.
- In `main`, build several laps (mixing `Duration::from`, a parsed string, and
  `Duration::new`), print each name padded to the widest name with its time,
  sum the durations with `+`, parse a race length, compare it with
  `is_longer_than`, and show an error case for a four-field string.
...
total: 00:04:40
race length: 01:00:00
race longer than total: true

parsing '1:2:3:4': Err("expected 1 to 3 ':'-separated fields: '1:2:3:4'")
```
...
trait Timed: fmt::Display {
    fn total_seconds(&self) -> u64;

    fn is_longer_than(&self, other: &impl Timed) -> bool {
...
impl FromStr for Duration {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if count == 0 || count > 3 {
            return Err(format!("expected 1 to 3 ':'-separated fields: '{s}'"));
        }
        Ok(Duration { seconds: total })
    }
}

impl Timed for Duration {
    fn total_seconds(&self) -> u64 {
        self.seconds
    }
}
...
impl AsRef<str> for Lap {
    fn as_ref(&self) -> &str {
        &self.name
    }
}

fn widest_name(laps: &[impl AsRef<str>]) -> usize {
    let mut max = 0;
    for lap in laps {
        let len = lap.as_ref().len();
        if len > max {
            max = len;
        }
    }
...
fn main() {
    let laps = vec![
        Lap::new("warmup", Duration::from(90)),
        Lap::new("sprint", "01:05".parse().unwrap()),
        Lap::new("cooldown", Duration::new(125)),
    ];

    println!("--- Laps ---");
    let width = widest_name(&laps);
    for lap in &laps {
        println!("{:<width$} {}", lap.as_ref(), lap.elapsed);
    }

    let mut total = Duration::new(0);
    for lap in &laps {
        total = total + lap.elapsed;
    }
    println!("\ntotal: {total}");

    let race: Duration = "1:00:00".parse().unwrap();
    println!("race length: {race}");
    println!("race longer than total: {}", race.is_longer_than(&total));

    let bad: Result<Duration, _> = "1:2:3:4".parse();
    println!("\nparsing '1:2:3:4': {bad:?}");
}
```
