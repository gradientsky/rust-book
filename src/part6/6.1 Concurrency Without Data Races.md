...
### Try It: Spawning Threads

These short exercises practice spawning threads, returning values from them, and moving ownership across the boundary, each solvable in a single `main`.

#### Exercise 6.1-A1: Parallel Half-Sum

Use `thread::scope` to sum an array in two halves at once. Split the slice with `split_at` at its midpoint, spawn one scoped thread per half (each returns its partial sum from the closure), then `join` both handles and add the two partials together. Because the threads only read the borrowed slice, no `move` and no `Arc` are needed.

Expected output:

```
first half + second half = 700
```
...
fn main() {
    let prices = [120, 80, 200, 60, 150, 90];

    let total: u32 = thread::scope(|s| {
        let (left, right) = prices.split_at(prices.len() / 2);

        let left_handle = s.spawn(|| left.iter().sum::<u32>());
        let right_handle = s.spawn(|| right.iter().sum::<u32>());

        left_handle.join().unwrap() + right_handle.join().unwrap()
    });

    println!("first half + second half = {total}");
}
```
...
#### Exercise 6.1-A2: Returning a Value from a Thread

A spawned thread can compute a value and hand it back through its `JoinHandle`. Move a `String` into a `thread::spawn` closure, count its whitespace-separated words inside the thread, and return a formatted summary `String`. Back in `main`, recover that `String` with `handle.join().unwrap()` and print it. The `move` keyword transfers ownership of the report into the thread.
...
fn main() {
    let report = String::from("quarterly numbers");

    let handle = thread::spawn(move || {
        let word_count = report.split_whitespace().count();
        format!("processed {word_count} words")
    });
...
### Try It: Shared State and Atomics

These short exercises practice the `Arc<Mutex<T>>` pattern and lock-free atomic counters, each solvable in a single `main`.

#### Exercise 6.1-B1: Shared Append Log

Wrap a `Vec<i32>` in `Arc<Mutex<...>>` and let four threads each push one value into it. Spawn four threads (worker ids `0..4`); each `Arc::clone`s the log, locks it, and pushes `worker * 10`. After joining every handle, lock the vector one last time, sort it (the push order across threads is nondeterministic, so sort before printing), and print the contents and their sum.

Expected output:

```
collected: [0, 10, 20, 30]
total: 60
```
...
fn main() {
    ...
    println!("total: {}", entries.iter().sum::<i32>());
}
```
...
#### Exercise 6.1-B2: Atomic Threshold Counter

Count how many numbers exceed a threshold using a lock-free `AtomicUsize`. Share a `Vec<i32>` behind an `Arc` and an `Arc<AtomicUsize>` counter across threads. Spawn one thread per two-element chunk; each thread inspects its slice and calls `fetch_add(1, Ordering::Relaxed)` for every value greater than ten. After all threads join, read the final count with `load(Ordering::Relaxed)`. The total is the same regardless of the order the threads finish in.
...
fn main() {
    let numbers = Arc::new(vec![3, 8, 1, 12, 5, 20, 7, 16]);
    let over_ten = Arc::new(AtomicUsize::new(0));

    let mut handles = vec![];
    for chunk_start in (0..numbers.len()).step_by(2) {
        let numbers = Arc::clone(&numbers);
        let over_ten = Arc::clone(&over_ten);
        let handle = thread::spawn(move || {
            for &n in &numbers[chunk_start..chunk_start + 2] {
                if n > 10 {
                    over_ten.fetch_add(1, Ordering::Relaxed);
                }
            }
        });
        handles.push(handle);
    ...
    println!("values over ten: {}", over_ten.load(Ordering::Relaxed));
}
```
...
These short exercises practice sending values between threads through `mpsc` channels, each solvable in a single `main`.

#### Exercise 6.1-C1: Squares Down a Channel

Send a stream of computed values from a worker thread to the main thread. Create a channel with `mpsc::channel`, move the `Sender` into a spawned thread, and send the squares of `1..=5`. In `main`, collect the `Receiver` into a `Vec` (the `for`/iterator loop ends when the sender is dropped at the thread's exit) and print the values and their sum. Values arrive in send order on a single channel.

Expected output:

```
received: [1, 4, 9, 16, 25]
sum: 55
```
...
fn main() {
    ...
    println!("received: {squares:?}");
    println!("sum: {}", squares.iter().sum::<i32>());
}
```
...
#### Exercise 6.1-C2: Multiple Producers

Feed one receiver from several sender threads. Clone the `Sender` once per thread (`0..3`), have each thread send `id * 100`, then `drop` the original `tx` so the channel closes once every clone is gone. Collect the receiver into a `Vec`, sort it (the arrival order across producers is nondeterministic), and print the results and the count. Forgetting the `drop(tx)` would leave the channel open and the collect would block forever.
...
fn main() {
    ...
    for id in 0..3 {
        let tx = tx.clone();
        thread::spawn(move || {
            tx.send(id * 100).unwrap();
        });
    }

    drop(tx);

    let mut totals: Vec<i32> = rx.iter().collect();
    totals.sort();
    println!("results: {totals:?}");
    println!("count: {}", totals.len());
}
```
...
### Exercise 6.1-1: Parallel Document Analyzer
...
### Exercise 6.1-2: Concurrent Ballot Box
...
### Exercise 6.1-4: Parallel Image Tiler

Build a grayscale image processor that brightens an image in parallel by handing
each row to its own scoped thread. This exercises `thread::scope`, disjoint
mutable borrowing with `split_at_mut`, `saturating_mul`, and collecting
per-thread reports into a sorted summary.

**Concepts practiced:** `thread::scope`, scoped `s.spawn`, disjoint mutable
slices via `split_at_mut`, `move` closures over mutable borrows, `iter_mut`,
`saturating_mul`, `sort_by_key`, result collection from join handles.

Build these pieces:

- A `RowReport` struct with `id: usize`, `bright: usize`, and `brightest: u8`.
- A `process_row(id: usize, row: &mut [u8]) -> RowReport` function that walks
  the row, tracks the brightest pixel, counts pixels at or above `128`, and
  then doubles every pixel in place with `saturating_mul(2)`.
- In `main`, define a `WIDTH` of `6` and a flattened `2x6` image as a
  `Vec<u8>`:
  ```rust
  let mut image: Vec<u8> = vec![
      10, 200, 30, 140, 5, 90,    // row 0
      130, 8, 250, 12, 160, 7,    // row 1
  ];
  ```
- Use `thread::scope` and `image.split_at_mut(WIDTH)` to split the buffer into
  two disjoint, mutable rows, then give one to each scoped thread. The compiler
  proves the halves never overlap, so no two threads touch the same pixel.
- Collect the `RowReport`s, sort them by `id`, print per-row stats, then print a
  summary: total pixels, total bright pixels, peak brightness before the boost,
  and the buffer after the boost.

Expected output:

```
processed 2 rows in parallel:
  row 0: 2 bright, brightest = 200
  row 1: 3 bright, brightest = 250

summary:
  total pixels: 12
  bright pixels (>= 128): 5
  peak brightness before boost: 250
  buffer after boost: [20, 255, 60, 255, 10, 180, 255, 16, 255, 24, 255, 14]
```
...
fn process_row(id: usize, row: &mut [u8]) -> RowReport {
    let mut bright = 0;
    ...
    for value in row.iter_mut() {
        ...
        // Boost contrast: saturating doubling, applied in place.
...
fn main() {
    ...
    // A 2x6 grayscale image flattened into one row-major buffer.
    let mut image: Vec<u8> = vec![
        10, 200, 30, 140, 5, 90, // row 0
        130, 8, 250, 12, 160, 7, // row 1
    ];

    let row_count = image.len() / WIDTH;

    // Split the buffer into two disjoint mutable halves, one row each. The
    // compiler proves the halves never overlap, so no two threads touch the
    // same pixel.
    let mut reports: Vec<RowReport> = thread::scope(|s| {
        let (row0, row1) = image.split_at_mut(WIDTH);

        let h0 = s.spawn(move || process_row(0, row0));
        let h1 = s.spawn(move || process_row(1, row1));
    ...
    println!("processed {row_count} rows in parallel:");
    let mut total_bright = 0;
    let mut peak = 0u8;
    for r in &reports {
        println!(
            "  row {}: {} bright, brightest = {}",
            r.id, r.bright, r.brightest
        );
        total_bright += r.bright;
        peak = peak.max(r.brightest);
    }

    println!("\nsummary:");
    println!("  total pixels: {}", image.len());
    println!("  bright pixels (>= 128): {total_bright}");
    println!("  peak brightness before boost: {peak}");
    println!("  buffer after boost: {image:?}");
}
```

</details>

### Exercise 6.1-5: Endpoint Health Checker

Build a parallel health checker where worker threads probe a set of service
endpoints against a shared latency budget, tally results with lock-free
counters, and trip a circuit breaker when an endpoint is badly degraded. This
exercises `Arc<RwLock<T>>` for read-mostly shared configuration, `AtomicUsize`
counters, and an `AtomicBool` flag.

**Concepts practiced:** `Arc<RwLock<T>>`, `RwLock::read`, `thread::scope`,
`Arc::clone`, `AtomicUsize::fetch_add`, `AtomicBool::store`, `Ordering::Relaxed`,
`flat_map` over join handles, `sort_by` with `then`, `split_at` for fixed batching.

Build these pieces:

- A `Limits` struct with `latency_budget_ms: u32` (derive `Clone`), wrapped in
  `Arc<RwLock<Limits>>`. An endpoint is healthy when its latency is at or below
  the budget.
- An `Outcome` struct with `name: &'static str`, `latency_ms: u32`, and
  `healthy: bool`.
- A fixed list of probe results as `Vec<(&'static str, u32)>`:
  - `("auth", 40)`, `("billing", 220)`, `("search", 90)`, `("cache", 15)`,
    `("media", 310)`, `("profile", 70)`.
- Three shared atomics in `Arc`: a `healthy` and a `degraded` `AtomicUsize`, and
  a `breaker_tripped` `AtomicBool`.
- Split the endpoints into two fixed batches at the midpoint. Use
  `thread::scope` to spawn one worker per batch. Each worker reads the budget
  from the `RwLock`, classifies each endpoint, increments the right counter, and
  stores `true` in the breaker flag when a latency exceeds twice the budget. It
  returns its local `Vec<Outcome>`.
- Flatten the per-worker results, sort by latency (then by name), and print each
  endpoint with an `ok`/`DEGRADED` tag, the healthy/degraded summary, and the
  circuit-breaker state.

Expected output:

```
health check (budget = 150 ms):
  cache     15 ms  [ok]
  auth      40 ms  [ok]
  profile   70 ms  [ok]
  search    90 ms  [ok]
  billing  220 ms  [DEGRADED]
  media    310 ms  [DEGRADED]

summary: 4 healthy, 2 degraded
circuit breaker: TRIPPED (an endpoint exceeded twice the budget)
```
...
#[derive(Clone)]
struct Limits {
    // An endpoint is healthy when its latency is at or below this budget.
...
fn main() {
    // Simulated probe results: (endpoint name, measured latency in ms).
    // Treated as fixed inputs so the run is fully deterministic.
    let endpoints: Vec<(&'static str, u32)> = vec![
        ("auth", 40),
        ("billing", 220),
        ("search", 90),
        ("cache", 15),
    ...
    // Shared, read-mostly configuration behind an RwLock.
    let limits = Arc::new(RwLock::new(Limits {
        latency_budget_ms: 150,
    }));

    // Lock-free shared counters and a circuit-breaker flag.
    let healthy = Arc::new(AtomicUsize::new(0));
    let degraded = Arc::new(AtomicUsize::new(0));
    let breaker_tripped = Arc::new(AtomicBool::new(false));

    // Split the endpoints into two fixed batches, one per worker thread.
    // split_at hands back two borrowed slices — no copying — and scoped
    // threads are allowed to borrow them.
    let mid = endpoints.len() / 2;
    let (first, second) = endpoints.split_at(mid);
    let batches = [first, second];

    let mut results: Vec<Outcome> = thread::scope(|s| {
        let handles: Vec<_> = batches
            ...
            .map(|batch| {
                ...
                s.spawn(move || {
                    ...
                    for &(name, latency_ms) in batch {
                        // Many readers can hold the budget at once.
                        let budget = limits.read().unwrap().latency_budget_ms;
                        let ok = latency_ms <= budget;
                        if ok {
                            healthy.fetch_add(1, Ordering::Relaxed);
                        } else {
                            degraded.fetch_add(1, Ordering::Relaxed);
                            // Latency more than double the budget trips the breaker.
                            if latency_ms > budget * 2 {
                                breaker_tripped.store(true, Ordering::Relaxed);
                            }
                        }
                        local.push(Outcome {
                            name,
    ...
    results.sort_by(|a, b| a.latency_ms.cmp(&b.latency_ms).then(a.name.cmp(b.name)));

    let budget = limits.read().unwrap().latency_budget_ms;
    println!("health check (budget = {budget} ms):");
    for o in &results {
        let status = if o.healthy { "ok" } else { "DEGRADED" };
        println!("  {:<8} {:>3} ms  [{status}]", o.name, o.latency_ms);
    }

    let healthy = healthy.load(Ordering::Relaxed);
    let degraded = degraded.load(Ordering::Relaxed);
    println!("\nsummary: {healthy} healthy, {degraded} degraded");

    if breaker_tripped.load(Ordering::Relaxed) {
        println!("circuit breaker: TRIPPED (an endpoint exceeded twice the budget)");
    } else {
...
### Exercise 6.1-6: Log Event Aggregator

Build a fan-in log aggregator where several service threads each emit a batch of
events into one shared channel, and the main thread tallies them by severity.
This exercises the multiple-producer pattern (cloned `Sender`, dropping the
original), `mpsc` single-consumer fan-in, an `Arc<AtomicUsize>` shared counter,
and deterministic reporting from a `HashMap`.

**Concepts practiced:** `mpsc::channel`, cloning a `Sender` for multiple
producers, `drop(tx)` to close the channel, `thread::spawn` with `move`,
ownership transfer through a channel, `Arc<AtomicUsize>` with `fetch_add`,
`HashMap` entry API with `or_default`, methods on an enum via `match self`,
`sort_by_key` and `sort` for deterministic output.

Build these pieces:

- A `Level` enum (`Info`, `Warn`, `Error`) deriving `Clone, Copy, PartialEq, Eq,
  Hash`, with a `label(self) -> &'static str` method (`"INFO"`/`"WARN"`/
  `"ERROR"`) and a `rank(self) -> u8` method that orders `Error` before `Warn`
  before `Info`.
- An `Event` struct with `source: &'static str`, `level: Level`, and
  `message: &'static str`.
- A fixed list of sources, each paired with a `Vec<(Level, &'static str)>`
  batch:
  - `"gateway"`: info "request accepted", warn "slow upstream", error "upstream timeout"
  - `"worker"`: info "job started", info "job finished"
  - `"db"`: warn "connection pool low", error "deadlock detected"
- Create one `mpsc::channel::<Event>()` and an `Arc<AtomicUsize>` named
  `emitted`. Spawn one producer thread per source: each clones the sender, sends
  its events, and bumps the counter with `fetch_add`. Drop the original sender
  after spawning so the receive loop terminates.
- In the single consumer (main thread), iterate the receiver, tally counts per
  `Level` with the entry API, and collect every `Error` event's source and
  message. Sort the tally by `rank`, sort the error list, and print the
  aggregated counts, the error detail, and the total events emitted.
...
fn main() {
    // Each service is a producer with a fixed batch of log events.
    let sources: Vec<(&'static str, Vec<(Level, &'static str)>)> = vec![
        (
            "gateway",
            vec![
                (Level::Info, "request accepted"),
                (Level::Warn, "slow upstream"),
                (Level::Error, "upstream timeout"),
            ],
        ),
        (
            "worker",
            vec![
                (Level::Info, "job started"),
                (Level::Info, "job finished"),
            ],
        ),
        (
            "db",
            vec![
                (Level::Warn, "connection pool low"),
                (Level::Error, "deadlock detected"),
            ],
        ),
    ];
    ...
    // Spawn one producer per source; each clones the sender so the channel
    // stays open until every producer has finished.
    ...
    // Drop the original sender so the receive loop ends once producers are done.
    drop(tx);

    // Single consumer: fan-in. Tally by level and keep the error log.
    ...
    for event in rx {
        *counts.entry(event.level).or_default() += 1;
        if event.level == Level::Error {
            errors.push((event.source, event.message));
        }
    }

    let mut tally: Vec<(Level, usize)> = counts.into_iter().collect();
    tally.sort_by_key(|(level, _)| level.rank());

    println!("aggregated log events:");
    for (level, count) in &tally {
        println!("  {:<5} {count}", level.label());
    }
