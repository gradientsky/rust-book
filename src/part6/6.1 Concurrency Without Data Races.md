...
- **Shared state** (`Arc<Mutex<T>>`) is natural when multiple threads need to
  read and update the same value, like a counter, a cache, or a configuration.
...
- **`Arc<Mutex<T>>`** is the standard pattern for shared mutable state: `Arc`
  for shared ownership, `Mutex` for exclusive access.
