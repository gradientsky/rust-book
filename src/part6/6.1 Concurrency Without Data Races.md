...
`Arc<T>` — short for _atomically reference-counted_ — works like `Rc<T>` from
Part 5, but its reference count uses atomic operations that are safe across
...
This is the borrowing rule from Part 2, applied at runtime. A `RwLock` allows
...
`RwLock`. A simple counter or a task queue is better served by `Mutex`.
...
The Rust 2024 edition introduces _async closures_ — closures that return
futures:

```rust,ignore
// Requires nightly or Rust 2024 edition
...
Before Rust 2024, you had to write `|url| async move { ... }`, which was
awkward because the closure and the future had separate capture semantics. Async
closures fix this: the closure itself is async, so captures work naturally.

Async closures implement one of three new traits — `AsyncFn`, `AsyncFnMut`, or
`AsyncFnOnce` — mirroring the `Fn`, `FnMut`, `FnOnce` hierarchy you learned in
Part 2. This makes it possible to write higher-order async functions that accept
async closures as arguments:

```rust,ignore
async fn retry<F>(action: F, attempts: u32) -> Result<String, String>
where
...
This is a significant ergonomic improvement for async Rust. The `AsyncFn` trait
bound reads naturally, and the compiler handles the complex interaction between
closures and futures automatically.
...
- **`Arc<Mutex<T>>`** is the standard pattern for shared mutable state: `Arc`
  ...
  many concurrent readers when writes are infrequent.
- **Channels** (`mpsc::channel`) pass values between threads by transferring
  ownership. Prefer channels when the design is a pipeline.
- **`async`/`await`** enables lightweight concurrency for I/O-bound work. You
  need a runtime like tokio. Async closures are new in Rust 2024.
