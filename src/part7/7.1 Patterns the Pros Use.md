...
fn main() {
    ...
    println!("{}:{} (tls: {})", server.host, server.port, server.tls_enabled);
}
```

```
0.0.0.0:3000 (tls: true)
```
...
```
Ok(DatabaseConfig { url: "postgres://localhost/mydb", pool_size: 10 })
...
The same combinators exist on `Result`, with the error side passing through
unchanged. You have already used many of these — this table is a reminder that
`match` is not your only tool.
...
Total words: 25
Longest section: "Type-state encodes valid transitions in the type system." (9 words)
...
- **Builder** separates construction from the type itself. Use consuming builders
  with method chaining for clean configuration APIs.
